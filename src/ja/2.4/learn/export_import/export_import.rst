...
| 本シナリオでは、ワークスペース admin1 をエクスポート元のワークスペース、ワークスペース admin2 をインポート先のワークスペースとして進めていきます。
...
| まずは ワークスペース admin1 での作業になります。

パラメータシートの作成
----------------------

| まずは、データをエクスポートするためにパラメータシートを作成していきましょう。
...
| エクスポート/インポートには、環境移行と時刻指定とExcel一括の3つのモードがあります。詳しくは :doc:`../../../manuals/export_import/menu_export_import` を参照してください。
| まずは、環境移行モードで実行します。
...
| 次に時刻指定モードで実行します。詳しくは :doc:`../../../manuals/export_import/menu_export_import` を参照してください。

作業準備
--------

| 再度ワークスペース admin1 での作業となります。
...
| 今回は時刻指定モードでエクスポートを行うため、以下のパラメータ更新は時間を空けて別々に行います。（2~3分程度）各パラメータの更新後、:menuselection:`入力用 --> ディレクトリ設定 --> フィルタ` 最終更新日時を確認しておきましょう。
...
| 次にExcel一括モードで実行します。詳しくは :doc:`../../../manuals/export_import/excel_export_import` を参照してください。
